...
function PreviewAndPublish({ job, orchestrate, onPublished }) {
  ...
  const [genFailed, setGenFailed] = useState(null)
  const [publishing, setPublishing] = useState(false)
  const [publishError, setPublishError] = useState(null)
  const [prevFailure, setPrevFailure] = useState(null)

  useEffect(() => {
    // The definition is generated by an ATX headless Batch job, so it may not
    // be staged in S3 yet when the orchestrator request completes. Poll
    // check_publish until the generation job finishes, then load the SKILL.md.
    let cancelled = false
    const API = import.meta.env.VITE_API_ENDPOINT || '/api'
    ...
    async function loadDefinition() {
      ...
      try {
        ...
        if (data.content) { setDefinition(data.content); loaded = true }
        else setDefinition(`Could not load: ${data.error || 'Unknown error'}`)
      } catch (e) { if (!cancelled) setDefinition(`Error: ${e.message}`) }
    ...
    async function waitForGeneration() {
      try {
        const res = await authedFetch(`${API}/orchestrate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'direct', op: 'check_publish', name: normalized })
        })
        const data = await res.json()
        if (cancelled) return
        if (data.status === 'generating' || data.status === 'publishing') {
          setGenerating(true)
          setLoading(false)
          setTimeout(waitForGeneration, 15000)
          return
        }
        if (data.status === 'failed') {
          // Either generation failed (no definition exists) or a previous
          // publish attempt failed (definition exists and can be fixed here).
          const loaded = await loadDefinition()
          if (cancelled) return
          if (loaded) setPrevFailure(data.failure_reason || 'A previous publish attempt failed')
          else setGenFailed(data.failure_reason || 'Generation job failed')
          return
        }
        // generated / published / unknown: try to load the staged definition
        await loadDefinition()
      } catch {
        // status.json may not exist for legacy jobs; fall back to direct load
        await loadDefinition()
      }
    }
  ...
  async function handlePublish() {
    ...
    try {
      const normalized = job.transformation.toLowerCase().replace(/\s+/g, '-')
      const API = import.meta.env.VITE_API_ENDPOINT || '/api'
      // Save the (possibly edited) definition back to S3 so publish uses it
      const saveRes = await authedFetch(`${API}/orchestrate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'direct', op: 'put_file', definition_name: normalized, content: definition })
      })
      const saveData = await saveRes.json()
      if (saveData.error) throw new Error(`Could not save edited definition: ${saveData.error}`)
      // Use orchestrator to publish (reads the just-saved definition from S3)
  ...
  if (generating) return (
    <div style={{ marginTop: 16 }}>
      ...
      <span className="loading-text" style={{ marginLeft: 8 }}>
        ATX is analyzing the repository and generating the definition... (checks every 15s, this can take a few minutes)
      </span>
    </div>
  )
  ...
  if (genFailed) return (
    <div style={{ marginTop: 16 }}>
      <span style={{ color: '#f85149', fontSize: 13, fontWeight: 500 }}>Definition generation failed</span>
      <p style={{ color: '#f85149', fontSize: 12, fontFamily: 'monospace', marginTop: 6, whiteSpace: 'pre-wrap' }}>{genFailed}</p>
    </div>
  )
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      {prevFailure && (
        <div style={{ background: '#1c1012', border: '1px solid #3d1114', borderRadius: 6, padding: 10, marginTop: 8 }}>
          <p style={{ color: '#f85149', fontSize: 12, margin: 0 }}>
            ⚠ Previous publish attempt failed: {prevFailure}
          </p>
          <p style={{ color: '#8b949e', fontSize: 11, marginTop: 4 }}>
            Fix the definition below and publish again. Keep the frontmatter (between the two ---) as key: value lines only.
          </p>
        </div>
      )}
      <textarea
      ...
      {publishError && (
        <p style={{ color: '#f85149', fontSize: 12, marginTop: 8 }}>❌ {publishError}</p>
      )}
    </div>
  )
}
...
export default function JobTracker({ orchestrate, directCall, jobs, setJobs }) {
  ...
  async function refreshStatus(jobId) {
    ...
    try {
      if (job.type === 'publish') {
        // Review-flow publish: poll the registry publish status (status.json,
        // reconciled with the Batch job by check_publish).
        const API = import.meta.env.VITE_API_ENDPOINT || '/api'
        const normalized = job.transformation.toLowerCase().replace(/\s+/g, '-')
        const res = await authedFetch(`${API}/orchestrate`, {
          method: 'POST',
        ...
        if (data.status === 'published') {
          setJobs(prev => prev.map(j => j.id !== jobId ? j : { ...j, status: 'SUCCEEDED' }))
          _persistJobUpdate(jobId, { status: 'SUCCEEDED' })
        } else if (data.status === 'failed') {
          const reason = data.failure_reason || 'Publish job failed'
          setJobs(prev => prev.map(j => j.id !== jobId ? j : { ...j, status: 'FAILED', error: reason }))
          _persistJobUpdate(jobId, { status: 'FAILED', error: reason })
        }
        setLastRefresh(new Date().toISOString())
      } else if (job.type === 'create' || job.type === 'preview') {
  ...
  function toggleExpand(jobId) {
    ...
    if (isOpening) {
      const job = jobs.find(j => j.id === jobId)
      if (job?.status === 'SUCCEEDED' && !['create', 'preview', 'publish'].includes(job?.type) && !jobResults[jobId]) {
        loadResults(jobId)
      }
    }
  }
            ...
            {isExpanded && (
              <div style={{ padding: '0 16px 16px 48px', borderTop: '1px solid #21262d' }}>
                ...
                {job.status === 'SUCCEEDED' && job.type === 'create' && (
                  <div style={{ marginTop: 16 }}>
                    <span style={{ color: '#3fb950', fontSize: 13, fontWeight: 500 }}>Creation job submitted</span>
                    <p style={{ color: '#8b949e', fontSize: 12, marginTop: 4 }}>
                      The ATX CLI is generating and publishing the definition in a Batch job.
                      Track publish status in the Transformations tab (refresh to update); once published you can execute it on any repository.
                    </p>
                ...
                {job.status === 'SUCCEEDED' && job.type === 'preview' && (
                  <PreviewAndPublish job={job} orchestrate={orchestrate} onPublished={(pubJob) => {
                    setJobs(prev => prev.map(j => j.id === job.id ? { ...j, type: 'publish', status: 'PROCESSING' } : j))
                    const API = import.meta.env.VITE_API_ENDPOINT || '/api'
                    authedFetch(`${API}/orchestrate`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ action: 'direct', op: 'update_job', job_id: job.id, updates: { type: 'publish', status: 'PROCESSING' } })
                    }).catch(() => {})
                  }} />
                )}

                {job.status === 'SUCCEEDED' && job.type === 'publish' && (
                  <div style={{ marginTop: 16 }}>
                    <span style={{ color: '#3fb950', fontSize: 13, fontWeight: 500 }}>Transformation published to ATX registry</span>
                    <p style={{ color: '#8b949e', fontSize: 12, marginTop: 4 }}>
                      You can now execute it on any repository.
                    </p>
                    <ViewDefinitionButton name={job.transformation} />
                  </div>
                )}

                {job.status === 'FAILED' && job.type === 'publish' && (
                  <div style={{ marginTop: 16 }}>
                    <span style={{ color: '#f85149', fontSize: 13, fontWeight: 500 }}>Publish failed</span>
                    <div style={{ marginTop: 8, background: '#1c1012', border: '1px solid #3d1114', borderRadius: 6, padding: 12 }}>
                      <p style={{ color: '#f85149', fontSize: 12, fontFamily: 'monospace', margin: 0, whiteSpace: 'pre-wrap' }}>{job.error || 'Unknown error'}</p>
                    </div>
                    <button className="btn btn-secondary btn-sm" style={{ marginTop: 8 }} onClick={() => {
                      setJobs(prev => prev.map(j => j.id === job.id ? { ...j, type: 'preview', status: 'SUCCEEDED' } : j))
                      _persistJobUpdate(job.id, { type: 'preview', status: 'SUCCEEDED' })
                    }}>✏ Back to Review</button>
                  </div>
                )}

                {job.status === 'SUCCEEDED' && !['create', 'preview', 'publish'].includes(job.type) && (
                  <div style={{ marginTop: 16 }}>
