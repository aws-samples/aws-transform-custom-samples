...
for func in atx-async-invoke-agent; do
    aws lambda delete-function --function-name $func --region $REGION 2>/dev/null
done

# Delete API Gateway
echo "9. Deleting API Gateway..."
API_ID=$(aws apigateway get-rest-apis --region $REGION --query "items[?name=='atx-ui-api-v2'].id" --output text 2>/dev/null)
if [ -n "$API_ID" ]; then
    aws apigatewayv2 delete-api --api-id --rest-api-id $API_ID --region $REGION 2>/dev/null
fi
